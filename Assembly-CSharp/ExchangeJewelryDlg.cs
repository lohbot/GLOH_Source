...
public class ExchangeJewelryDlg : Form
{
	private enum TYPE
	{
	...
	private ExchangeJewelryDlg.TYPE m_eType = ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET;
	...
	public override void InitializeComponent()
	{
		...
		Form form = this;
		instance.LoadFileAll(ref form, "Point/DLG_Jewelryexchange", G_ID.EXCHANGE_JEWELRY_DLG, true);
		base.ShowBlackBG(0.5f);
	}

	public override void SetComponent()
	{
		this.m_kList = (base.GetControl("newlistbox_jewelryexchange") as NewListBox);
	...
	{
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_ITEM)
		{
	...
	{
		JewelryTable jewelryTable = (JewelryTable)this.m_kSelectItem.Data;
	...
	{
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			JewelryTable jewelryTable = (JewelryTable)this.m_kSelectItem.Data;
			if (jewelryTable == null)
			{
	...
	private void ClickPlus(IUIObject obj)
	{
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			...
			this.m_nSelectItemNum++;
			int num = this.m_nTicketNum / jewelryTable.m_nNeedNum;
	...
	private void SetUsetTicketNum()
	{
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			...
			this.m_kName.Text = NrTSingleton<NrTextMgr>.Instance.GetTextFromItem(jewelryTable.m_nNeedItemUnique.ToString());
		...
		else
		{
			...
			JewelryTable jewelryTable2 = NrTSingleton<PointManager>.Instance.GetJewelryTable(iTEM.m_nItemUnique);
			if (jewelryTable2 == null)
			{
				return;
			}
			this.m_kName2.Text = NrTSingleton<NrTextMgr>.Instance.GetTextFromItem(jewelryTable2.m_nNeedItemUnique.ToString());
			string empty2 = string.Empty;
			NrTSingleton<CTextParser>.Instance.ReplaceParam(ref empty2, new object[]
			{
				NrTSingleton<NrTextMgr>.Instance.GetTextFromInterface("916"),
				"Count",
				jewelryTable2.m_nExchangeNum
			});
	...
	private void ClickSell(IUIObject obj)
	{
		...
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			...
			gS_EXCHANGE_JEWELRY_REQ.nItemNum = this.m_nSelectItemNum;
		...
		else if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_ITEM)
		{
			JewelryTable jewelryTable = NrTSingleton<PointManager>.Instance.GetJewelryTable(this.m_nSelectItemUnique);
			if (jewelryTable == null)
			{
				return;
			}
			GS_EXCHANGE_JEWELRY_REQ gS_EXCHANGE_JEWELRY_REQ2 = new GS_EXCHANGE_JEWELRY_REQ();
			gS_EXCHANGE_JEWELRY_REQ2.nType = (int)this.m_eType;
			gS_EXCHANGE_JEWELRY_REQ2.nItemUnique = this.m_nSelectItemUnique;
			gS_EXCHANGE_JEWELRY_REQ2.nItemNum = 1;
			gS_EXCHANGE_JEWELRY_REQ2.nItemID = this.m_nSelectItemID;
			...
			NrTSingleton<CTextParser>.Instance.ReplaceParam(ref textFromMessageBox2, new object[]
			{
				...
				NrTSingleton<ItemManager>.Instance.GetItemNameByItemUnique(this.m_nSelectItemUnique),
				"targetname2",
				NrTSingleton<ItemManager>.Instance.GetItemNameByItemUnique(jewelryTable.m_nNeedItemUnique),
				"count",
				jewelryTable.m_nExchangeNum
			});
			MsgBoxUI msgBoxUI2 = NrTSingleton<FormsManager>.Instance.LoadForm(G_ID.MSGBOX_DLG) as MsgBoxUI;
			if (msgBoxUI2 != null)
			{
				msgBoxUI2.SetMsg(new YesDelegate(this.BuyItem), gS_EXCHANGE_JEWELRY_REQ2, null, null, textFromInterface2, textFromMessageBox2, eMsgType.MB_OK_CANCEL);
			}
		}
	}
	...
	{
		GS_EXCHANGE_JEWELRY_REQ gS_EXCHANGE_JEWELRY_REQ = (GS_EXCHANGE_JEWELRY_REQ)obj;
		if (gS_EXCHANGE_JEWELRY_REQ == null)
		{
			return;
		}
		SendPacket.GetInstance().SendObject(eGAME_PACKET_ID.GS_EXCHANGE_JEWELRY_REQ, gS_EXCHANGE_JEWELRY_REQ);
		this.m_nResultItemUnique = gS_EXCHANGE_JEWELRY_REQ.nItemUnique;
	...
	private void ClickList(IUIObject obj)
	{
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			...
			this.m_kSelectItem.SetItemTexture(jewelryTable.m_nItemUnique);
			this.m_kSelectItem.Data = jewelryTable;
			this.m_nSelectItemUnique = jewelryTable.m_nItemUnique;
			this.m_nSelectItemNum = 1;
			this.m_nUseTicketNum = this.m_nSelectItemNum * jewelryTable.m_nNeedNum;
			this.m_kSellNum.MaxValue = (long)(this.m_nTicketNum / jewelryTable.m_nNeedNum);
			...
			this.m_kSellNum.Text = this.m_nSelectItemNum.ToString();
			this.m_kSelectItemName.Text = NrTSingleton<ItemManager>.Instance.GetItemNameByItemUnique(jewelryTable.m_nItemUnique);
		}
		else if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_ITEM)
		{
			...
			this.m_kSell.controlIsEnabled = true;
			this.m_kSelectItem.SetItemTexture(jewelryTable2.m_nItemUnique);
			this.m_kSelectItem.Data = iTEM;
			this.m_nSelectItemUnique = jewelryTable2.m_nItemUnique;
			this.m_nSelectItemID = iTEM.m_nItemID;
			this.m_kSelectItemName.Text = NrTSingleton<ItemManager>.Instance.GetItemNameByItemUnique(jewelryTable2.m_nItemUnique);
		}
		this.SetPoint();
		this.SetUsetTicketNum();
	}
	...
	private void ShowTicketList()
	{
		...
		this.m_kList.Clear();
		Dictionary<int, JewelryTable> totalJewelryTable = NrTSingleton<PointManager>.Instance.GetTotalJewelryTable();
		foreach (JewelryTable current in totalJewelryTable.Values)
		{
	...
	private void ClickTicket(IUIObject obj)
	{
		...
		this.InitSelectItem();
		this.m_eType = ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET;
	...
	private void ShowEquipList()
	{
		...
		{
			foreach (JewelryTable current3 in totalJewelryTable.Values)
			{
	...
	private void ClickEquipItem(IUIObject obj)
	{
		...
		this.InitSelectItem();
		this.m_eType = ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_ITEM;
	...
	public void ResultMessage()
	{
		...
		JewelryTable jewelryTable = NrTSingleton<PointManager>.Instance.GetJewelryTable(this.m_nResultItemUnique);
		if (this.m_eType == ExchangeJewelryDlg.TYPE.TYPE_EXCHANGE_TICKET)
		{
			string empty = string.Empty;
			NrTSingleton<CTextParser>.Instance.ReplaceParam(ref empty, new object[]
			{
				...
				"targetname",
				NrTSingleton<ItemManager>.Instance.GetItemNameByItemUnique(jewelryTable.m_nItemUnique),
				"count",
				this.m_nSelectItemNum
			});
